defmodule StudyBotWeb.ChatLive do
  ...
  def handle_event("send_message", %{"query" => query}, socket) when query != "" do
    ...
    session = case current_session do
      nil ->
  ...
  def handle_info({:process_query, query, session_id}, socket) do
    ...
    case RAG.process_query(course_id, query, session_id) do
      {:ok, response, source} ->
        ...
        response_with_indicator = case source do
          :cached -> "🔄 " <> response
          :fallback -> "⚠️ " <> response
          :rag -> response
        end
              ...
              <div class="p-4 space-y-2">
                <%= for session <- @sessions do %>
                  <div class={[
                    ...
                    if(@current_session && @current_session.id == session.id, do: "bg-blue-100 border border-blue-200", else: "hover:bg-gray-100")
                  ]}>
                    <div class="flex justify-between items-start">
                      <div phx-click="select_session" phx-value-session_id={session.id} class="flex-1 min-w-0">
          ...
          <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <%= if @loading do %>
              <div class="flex justify-start">
                <div class="bg-white border border-gray-200 text-gray-800 max-w-3xl px-4 py-3 rounded-lg">
          ...
          <div class="border-t bg-white p-6">
            <form phx-submit="send_message" class="flex space-x-4">
              <input
                ...
                disabled={@loading}
                class="flex-1 border border-gray-300 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50"
                autofocus
              />
              <button
                type="submit"
